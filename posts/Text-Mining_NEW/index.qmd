...
After fitting all the models, I have decided to not fit the easy word embedding recipe (rec2), because of the long computing time! We have a large amount of data and extensive recipes, so when I raise the resamples for the prediction, R Studio quits the session.
...
According to the large amount of data, I have decided to not run the Null Model.

`#{r} mod0 <- null_model() %>% set_engine("parsnip") %>% set_mode("classification")`
...
So we can see a higher sensitivity and a high specificity
...
# 9. Prediction
## 9.1 Finalize the Workflow
```{r}
we_l1_wf3_final <- 
  we_l1_wf3 %>% 
  finalize_workflow(select_best(we_l1_fit3, metric = "roc_auc"))
```

## 5.2 Prediction On The Train Dataset
```{r}
options(mc.cores = parallel::detectCores())
fit_train <- we_l1_wf3_final %>% 
  fit(d_train)
```

## 5.3 Predictions On The Test Datensatz predicten
